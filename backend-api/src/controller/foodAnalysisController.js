...
export async function createUserFoodHistory(request, h) {
	...
	try {
		...
		await createFoodAnalysisService(userRef, {
			food: food,
			quantity: quantity,
			calories:
			...
			karbohidrat:
				(parseInt(foodData.komposisi_karbohidrat_g) * quantity) /
				parseInt(foodData.komposisi_per),
			protein:
				(parseInt(foodData.komposisi_lemak_g) * quantity) /
				parseInt(foodData.komposisi_per),
			lemak:
				(parseInt(foodData.komposisi_protein_g) * quantity) /
				parseInt(foodData.komposisi_per),
			date: Date.now(),
			mealtime: mealtime,
		});
