...
import {
	...
	fetchWorkoutsByName,
	changeWorkoutPlanCompleteStatusService,
...
export async function getRandomWorkout(request, h) {
	try {
		...
		if (target == 'Full') {
			const [upperWorkouts, lowerWorkouts] = await getFullBodyWorkout(option, h);
			...
			selectedWorkouts = workout.sort(() => 0.5 - Math.random()).slice(0, numOfWorkout);
		}
	...
	try {
		...
		if (!level || !target || !option) {
			return h
				.response({
					status: 400,
					message: 'Workout level, muscle target, and exercise option are required',
				})
				.code(400);
		}
		...
		return h
			.response({
				status: 500,
				message: 'An error occurred while creating a workout plan. Please try again later.',
			})
			.code(500);
	}
}
...
export async function updateUserWorkoutPlan(request, h) {
	try {
		...
		return h
			.response({
				status: 500,
				message: 'An error occurred while updating a workout plan. Please try again later.',
			})
			.code(500);
	}
}
...
export async function deleteUserWorkoutPlan(request, h) {
	try {
		...
		return h
			.response({
				status: 500,
				message: 'An error occurred while deleting a workout plan. Please try again later.',
			})
			.code(500);
	}
}
...
export async function getWorkoutRecommendations(request, h) {
	try {
		...
		const result = await getWorkoutPlanByTargetAndOption(userRef, target, option);
		...
		const workoutNames = result.workouts.map((workout) => workout.exercise_name);
...
export async function changeWorkoutPlanCompleteStatus(request, h) {
	try {
		const { uid } = request.auth;
		const userRef = doc(db, 'Users', uid);
		const userSnapshot = await getDoc(userRef);
		...
		const { planId } = request.params;
		await changeWorkoutPlanCompleteStatusService(userRef, planId);
		return h
			.response({
				status: 200,
				message: 'Workout plan updated successfully',
