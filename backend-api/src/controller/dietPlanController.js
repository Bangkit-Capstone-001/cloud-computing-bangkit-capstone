...
export async function getDietPlan(request, h) {
	try {
		...
		const intake = await getTodayTotalIntake(userRef);
		return h
			.response({
				status: 200,
				...
				data: {
					...
					calorieEaten: intake.calories,
					lemakEaten: intake.lemak,
					proteinEaten: intake.protein,
					karbohidratEaten: intake.karbohidrat,
					remainingCalories: Math.max(userDietPlan.calorie - intake.calories, 0),
					goal,
				},
			})
			.code(200);
...
export async function createDietPlan(request, h) {
	try {
		...
		if (!weightTarget) {
			...
			const { currentWeight, currentHeight, age, gender, goal, activityLevel } = userData;
			let bmr, calorie;
